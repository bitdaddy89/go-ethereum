...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Getd.LightScryptN, Getd.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.etherdata", new NodeConfig());
			node.start();
			...
			EtherdataClient ec = node.getEtherdataClient();
	...
	// signatures too. Regression test for go-etherdata issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "getd-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/etherdata/getdtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Getd and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.etherdata", "github.com/crypyto-panel/go-etherdata/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "getd.aar"), "getd.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "etherdata", "getdtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.etherdata.getdtest"
