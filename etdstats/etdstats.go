...
// Package etdstats implements the network stats reporting service.
package etdstats
...
import (
	...
	etdproto "github.com/crypyto-panel/go-etherdata/etd/protocols/etd"
...
// backend encompasses the bare-minimum functionality needed for etdstats reporting
...
// Service implements an Etherdata netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(etdstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["etd"]; info != nil {
		network = fmt.Sprintf("%d", info.(*etdproto.NodeInfo).Network)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the etdstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to etdstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to etdstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to etdstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to etdstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to etdstats")
