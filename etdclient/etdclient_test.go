...
// Verify that Client implements the etherdata interfaces.
var (
	_ = etherdata.ChainReader(&Client{})
	_ = etherdata.TransactionReader(&Client{})
	_ = etherdata.ChainStateReader(&Client{})
	_ = etherdata.ChainSyncReader(&Client{})
	_ = etherdata.ContractCaller(&Client{})
	_ = etherdata.GasEstimator(&Client{})
	_ = etherdata.GasPricer(&Client{})
	_ = etherdata.LogFilterer(&Client{})
	_ = etherdata.PendingStateReader(&Client{})
	// _ = etherdata.PendingStateEventer(&Client{})
	_ = etherdata.PendingContractCaller(&Client{})
)
...
func TestToFilterArg(t *testing.T) {
	...
	for _, testCase := range []struct {
		name   string
		input  etherdata.FilterQuery
		output interface{}
		err    error
	}{
		{
			"without BlockHash",
			etherdata.FilterQuery{
				Addresses: addresses,
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Etherdata Service
	config := &etdconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = etdash.ModeFake
	etdservice, err := etd.New(n, config)
	if err != nil {
		t.Fatalf("can't create new etherdata service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := etdservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == etherdata.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != etherdata.NotFound {
		t.Fatal("error should be etherdata.NotFound")
	}
}
