...
// Package p2p implements the Etherdata p2p network protocols.
package p2p
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "etherdata discovery")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		...
		if !tcp.IP.IsLoopback() && srv.NAT != nil {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "tcp", tcp.Port, tcp.Port, "etherdata p2p")
				srv.loopWG.Done()
			}()
		}
	}
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // Etherdata Node Record
