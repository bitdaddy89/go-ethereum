...
func (h *handler) syncTransactions(p *etd.Peer) {
	...
	// The etd/65 protocol introduces proper transaction announcements, so instead
...
func (h *handler) txsyncLoop64() {
	...
	send := func(s *txsync) {
		if s.p.Version() >= etd.ETD65 {
			panic("initial transaction syncer running on etd/65+")
		}
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *etd.Peer) bool {
	select {
...
func peerToSyncOp(mode downloader.SyncMode, p *etd.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync || op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Getd won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Getd manually and
		// restart) then it will be hard for Getd to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the fast sync. If changed, Getd
