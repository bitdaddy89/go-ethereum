...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [etd/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    etddb.Database  // Database to state sync into (and deduplicate via)
...
func New(checkpoint uint64, stateDb etddb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() etherdata.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < etd.ETD65 {
		return fmt.Errorf("%w: advertized %d < required %d", errTooOld, p.version, etd.ETD65)
	}
	mode := d.getMode()

	log.Debug("Synchronising with the network", "peer", p.id, "etd", p.version, "head", hash, "td", td, "mode", mode)
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (commonAncestor uint64, err error) {
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave sometding valid
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered sometding completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have sometding)
				// R: Nothing to give
				if mode != LightSync {
				...
				// peer gave us sometding useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if sometding failed in between processing chunks
				select {
				case <-d.cancelCh:
					rollbackErr = errCanceled
					return errCanceled
