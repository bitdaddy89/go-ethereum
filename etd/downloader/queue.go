...
type fetchRequest struct {
	...
	From    uint64          // [etd/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [etd/62] Requested headers, sorted by request order
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading sometding (sanity check to
	// not corrupt state)
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	// downloading sometding (sanity check not to corrupt state)
	if taskQueue.Empty() {
