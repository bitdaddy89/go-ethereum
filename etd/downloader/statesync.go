...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting sometding else, ignore the stale timeout.
...
func (s *stateSync) loop() (err error) {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for sometding to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < etddb.IdealBatchSize {
		return nil
	}
	start := time.Now()
	...
	for _, p := range peers {
		...
		if len(nodes)+len(codes) > 0 {
			...
			case s.d.trackStateReq <- req:
				req.peer.FetchNodeData(append(nodes, codes...)) // Unified retrieval under etd/6x
			case <-s.cancel:
			case <-s.d.cancelCh:
			}
		}
...
func (s *stateSync) process(req *stateReq) (int, error) {
	...
	for hash, task := range req.trieTasks {
		// If the node did deliver sometding, missing items may be due to a protocol
