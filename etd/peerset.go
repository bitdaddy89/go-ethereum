...
var (
	...
	// snap protocol without advertizing the etd main protocol.
	errSnapWithoutEth = errors.New("peer connected on snap without compatible etd support")
)

// peerSet represents the collection of active peers currently participating in
// the `etd` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*etdPeer // Peers connected on the `etd` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `etd` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `etd`
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers:    make(map[string]*etdPeer),
		snapWait: make(map[string]chan *snap.Peer),
		snapPend: make(map[string]*snap.Peer),
	}
}

// registerSnapExtension unblocks an already connected `etd` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `etd` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `etd` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `etd`
	if !peer.RunningCap(etd.ProtocolName, etd.ProtocolVersions) {
	...
	// Inject the peer into an `etd` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *etd.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `etd` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *etd.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *etdPeer {
	ps.lock.RLock()
...
func (ps *peerSet) peersWithoutBlock(hash common.Hash) []*etdPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*etdPeer, 0, len(ps.peers))
...
func (ps *peerSet) peersWithoutTransaction(hash common.Hash) []*etdPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
...
// len returns if the current number of `etd` peers in the set. Since the `snap`
// peers are tied to the existence of an `etd` connection, that will always be a
// subset of `etd`.
...
// difficulty.
func (ps *peerSet) peerWithHighestTD() *etd.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
