...
import (
	...
	"github.com/crypyto-panel/go-etherdata/etd/fetcher"
...
// support all the operations needed by the Etherdata chain protocols.
...
type handlerConfig struct {
	Database   etddb.Database            // Database for direct sync insertions
...
// newHandler returns a handler for all Etherdata chain management protocol.
...
// runEthPeer registers an etd peer into the joint etd/snap peerset, adds it to
// various subsistems and starts handling messages.
func (h *handler) runEthPeer(peer *etd.Peer, handler etd.Handler) error {
	...
	// Execute the Etherdata handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Etherdata handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Etherdata peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Etherdata peer registration failed", "err", err)
		return err
	}
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in etd syncer", "err", err)
		return err
	}
	if snap != nil {
...
// runSnapExtension registers a `snap` peer into the joint etd/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `etd`, all subsystem registrations and lifecycle management will be done by
// the main `etd` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Etherdata peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `etd` peer if it exists
	logger.Debug("Removing Etherdata peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Etherdata peer removal failed", "err", err)
	}
}
...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(2)
	go h.chainSync.loop()
	go h.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with etd/64.
}
...
func (h *handler) Stop() {
	...
	log.Info("Etherdata protocol stopped")
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*etdPeer][]common.Hash) // Set peer->hash to transfer directly
		annos = make(map[*etdPeer][]common.Hash) // Set peer->hash to announce
