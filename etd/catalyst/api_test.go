...
var (
	...
	// testAddr is the Etherdata address of the tester account.
...
func TestEth2AssembleBlock(t *testing.T) {
	...
	n, etdservice := startEthService(t, genesis, blocks[1:9])
	defer n.Close()

	api := newConsensusAPI(etdservice)
	signer := types.NewEIP155Signer(etdservice.BlockChain().Config().ChainID)
...
func TestEth2NewBlock(t *testing.T) {
	...
	n, etdservice := startEthService(t, genesis, blocks[1:5])
	defer n.Close()

	api := newConsensusAPI(etdservice)
	for i := 5; i < 10; i++ {
		p := executableData{
			ParentHash:   etdservice.BlockChain().CurrentBlock().Hash(),
	...
	exp := etdservice.BlockChain().CurrentBlock().Hash()
	...
	for i := 0; i < 4; i++ {
		...
		lastBlock, err = insertBlockParamsToBlock(etdservice.BlockChain().Config(), lastBlock.Header(), p)
		if err != nil {
			t.Fatal(err)
		}
	}

	if etdservice.BlockChain().CurrentBlock().Hash() != exp {
		t.Fatalf("Wrong head after inserting fork %x != %x", exp, etdservice.BlockChain().CurrentBlock().Hash())
	}
}

// startEthService creates a full node instance for testing.
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *etd.Etherdata) {
	t.Helper()
	...
	etdcfg := &etdconfig.Config{Genesis: genesis, Ethash: etdash.Config{PowMode: etdash.ModeFake}}
	etdservice, err := etd.New(n, etdcfg)
	if err != nil {
		t.Fatal("can't create etd service:", err)
	}
	if err := n.Start(); err != nil {
		t.Fatal("can't start node:", err)
	}
	if _, err := etdservice.BlockChain().InsertChain(blocks); err != nil {
		n.Close()
		t.Fatal("can't import test blocks:", err)
	}
	etdservice.SetEtherbase(testAddr)
