...
// Package catalyst implements the temporary etd1/etd2 RPC integration.
package catalyst
...
import (
	...
	chainParams "github.com/crypyto-panel/go-etherdata/params"
	"github.com/crypyto-panel/go-etherdata/rpc"
	"github.com/crypyto-panel/go-etherdata/trie"
)

// Register adds catalyst APIs to the node.
func Register(stack *node.Node, backend *etd.Etherdata) error {
...
func newConsensusAPI(etd *etd.Etherdata) *consensusAPI {
	return &consensusAPI{etd: etd}
}
...
func (api *consensusAPI) makeEnv(parent *types.Block, header *types.Header) (*blockExecutionEnv, error) {
	state, err := api.etd.BlockChain().StateAt(parent.Root())
	if err != nil {
		return nil, err
	}
	env := &blockExecutionEnv{
		chain:   api.etd.BlockChain(),
		state:   state,
		header:  header,
...
// data" required for etd2 clients to process the new block.
func (api *consensusAPI) AssembleBlock(params assembleBlockParams) (*executableData, error) {
	...
	coinbase, err := api.etd.Etherbase()
	if err != nil {
		return nil, err
	}
	num := parent.Number()
	...
	if config := api.etd.BlockChain().Config(); config.IsLondon(header.Number) {
		header.BaseFee = misc.CalcBaseFee(config, parent.Header())
	}
	err = api.etd.Engine().Prepare(bc, header)
	if err != nil {
		return nil, err
	}
	...
	// Create the block.
	block, err := api.etd.Engine().FinalizeAndAssemble(bc, header, env.state, transactions, nil /* uncles */, env.receipts)
	if err != nil {
		return nil, err
	}
...
func (api *consensusAPI) NewBlock(params executableData) (*newBlockResponse, error) {
	parent := api.etd.BlockChain().GetBlockByHash(params.ParentHash)
	if parent == nil {
		return &newBlockResponse{false}, fmt.Errorf("could not find parent %x", params.ParentHash)
	}
	block, err := insertBlockParamsToBlock(api.etd.BlockChain().Config(), parent.Header(), params)
	if err != nil {
		return nil, err
	}
	_, err = api.etd.BlockChain().InsertChainWithoutSealVerification(block)
