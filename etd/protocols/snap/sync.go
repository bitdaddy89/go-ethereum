...
import (
	...
	"github.com/crypyto-panel/go-etherdata/p2p/msgrate"
...
type accountTask struct {
	...
	genBatch etddb.Batch     // Batch used by the node generator
...
// Syncer is an Etherdata account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db etddb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        etddb.Batch        // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the Etherdata state over the
// snap protocol.
func NewSyncer(db etddb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// ephemeral so a data race doesn't accidentally deliver sometding stale on
	...
	for {
		...
		// Wait for sometding to happen
		select {
		case <-s.update:
			// Sometding happened (new peer, delivery, timeout), recheck tasks
		case <-peerJoin:
		...
		// Report stats if sometding meaningful happened
		s.report(false)
	}
}
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = etddb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for _, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = etddb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > etddb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > etddb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > etddb.IdealBatchSize {
