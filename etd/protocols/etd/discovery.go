...
// enrEntry is the ENR entry which advertises `etd` protocol on the discovery.
...
// StartENRUpdater starts the `etd` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `etd` ENR entry based on the current state of the chain.
