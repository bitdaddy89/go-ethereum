...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `etd` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `etd`.
...
// NodeInfo represents a short summary of the `etd` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Etherdata network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
...
// nodeInfo retrieves some `etd` protocol metadata about the running host node.
...
// Handle is invoked whenever an `etd` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `etd`", "err", err)
			return err
		}
	}
}
...
var etd65 = map[uint64]msgHandler{
...
var etd66 = map[uint64]msgHandler{
	NewBlockHashesMsg:             handleNewBlockhashes,
	NewBlockMsg:                   handleNewBlock,
	TransactionsMsg:               handleTransactions,
	NewPooledTransactionHashesMsg: handleNewPooledTransactionHashes,
	// etd66 messages with request-id
