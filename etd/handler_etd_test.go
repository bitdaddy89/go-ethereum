...
// on the `etd` protocol and convert them into a more easily testable form.
...
func (h *testEthHandler) TxPool() etd.TxPool                   { panic("no backing tx pool") }
func (h *testEthHandler) AcceptTxs() bool                      { return true }
func (h *testEthHandler) RunPeer(*etd.Peer, etd.Handler) error { panic("not used in tests") }
func (h *testEthHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testEthHandler) Handle(peer *etd.Peer, packet etd.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected etd packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit65(t *testing.T) { testForkIDSplit(t, etd.ETD65) }
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, etd.ETD66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		etdNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		etdProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Network:    1,
	...
	etdNoFork.Start(1000)
	etdProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer etdNoFork.Stop()
	defer etdProFork.Stop()
	...
	peerNoFork := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- etdNoFork.runEthPeer(peerProFork, func(peer *etd.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- etdProFork.runEthPeer(peerNoFork, func(peer *etd.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = etd.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = etd.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions65(t *testing.T) { testRecvTransactions(t, etd.ETD65) }
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, etd.ETD66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runEthPeer(sink, func(peer *etd.Peer) error {
		return etd.Handle((*etdHandler)(handler.handler), peer)
	})
...
func TestSendTransactions65(t *testing.T) { testSendTransactions(t, etd.ETD65) }
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, etd.ETD66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runEthPeer(src, func(peer *etd.Peer) error {
		return etd.Handle((*etdHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 65, 66:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post etd/65")
			}
...
func TestTransactionPropagation65(t *testing.T) { testTransactionPropagation(t, etd.ETD65) }
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, etd.ETD66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runEthPeer(sourcePeer, func(peer *etd.Peer) error {
			return etd.Handle((*etdHandler)(source.handler), peer)
		})
		go sink.handler.runEthPeer(sinkPeer, func(peer *etd.Peer) error {
			return etd.Handle((*etdHandler)(sink.handler), peer)
		})
	}
...
// Tests that post etd protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := etd.NewPeer(etd.ETD65, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := etd.NewPeer(etd.ETD65, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runEthPeer(local, func(peer *etd.Peer) error {
			return etd.Handle((*etdHandler)(handler.handler), peer)
		})
	}()
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := etd.NewPeer(etd.ETD65, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := etd.NewPeer(etd.ETD65, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock65(t *testing.T) { testBroadcastMalformedBlock(t, etd.ETD65) }
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, etd.ETD66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := etd.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runEthPeer(src, func(peer *etd.Peer) error {
		return etd.Handle((*etdHandler)(source.handler), peer)
	})
