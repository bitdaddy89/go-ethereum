...
// Package filters implements an etherdata filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  etherdata.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit etherdata.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit etherdata.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit etherdata.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit etherdata.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
