...
func BenchmarkFilters(b *testing.B) {
	...
	var (
		...
		addr3   = common.BytesToAddress([]byte("etherdata"))
		addr4   = common.BytesToAddress([]byte("random addresses please"))
	)
	defer db.Close()

	genesis := core.GenesisBlockForTesting(db, addr1, big.NewInt(1000000))
	chain, receipts := core.GenerateChain(params.TestChainConfig, genesis, etdash.NewFaker(), db, 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func TestFilters(t *testing.T) {
	...
	chain, receipts := core.GenerateChain(params.TestChainConfig, genesis, etdash.NewFaker(), db, 1000, func(i int, gen *core.BlockGen) {
		switch i {
		case 1:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
