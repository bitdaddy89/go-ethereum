...
// Package etdconfig contains the configuration of the ETD and LES protocols.
package etdconfig
...
// Defaults contains default settings for use on the Etherdata main net.
var Defaults = Config{
	SyncMode: downloader.SnapSync,
	Ethash: etdash.Config{
		CacheDir:         "etdash",
		CachesInMem:      2,
		CachesOnDisk:     3,
...
func init() {
	...
	if runtime.GOOS == "darwin" {
		...
		Defaults.Ethash.DatasetDir = filepath.Join(home, ".etdash")
	}
}

//go:generate gencodec -type Config -formats toml -out gen_config.go

// Config contains configuration options for of the ETD and LES protocols.
type Config struct {
	// The genesis block, which is inserted if the database is empty.
	// If nil, the Etherdata main net block is used.
	...
	// RPCGasCap is the global gas cap for etd-call variants.
	RPCGasCap uint64
...
func CreateConsensusEngine(stack *node.Node, chainConfig *params.ChainConfig, config *etdash.Config, notify []string, noverify bool, db etddb.Database) consensus.Engine {
	...
	case etdash.ModeFake:
		log.Warn("Ethash used in fake mode")
	case etdash.ModeTest:
		log.Warn("Ethash used in test mode")
	case etdash.ModeShared:
