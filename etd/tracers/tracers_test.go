...
import (
	...
	"github.com/crypyto-panel/go-etherdata/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Getd console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = etd.getBlock(etd.getTransaction(tx).blockHash);
  var genesis = etd.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.etd.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  etd.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
