...
func TestTraceCall(t *testing.T) {
	...
	var testSuite = []struct {
		...
		// Standard JSON trace upon the genesis, plain transfer.
		{
			...
			config:    nil,
			expectErr: nil,
			expect: &etdapi.ExecutionResult{
				Gas:         params.TxGas,
				Failed:      false,
				ReturnValue: "",
				StructLogs:  []etdapi.StructLogRes{},
			},
		},
	...
	var testSuite = []struct {
		...
		// Succcessful call with state overriding
		{
			...
			config: &TraceCallConfig{
				...
				StateOverrides: &etdapi.StateOverride{
					randomAccounts[0].addr: etdapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Ether)))},
				},
			},
			expectErr: nil,
			expect: &callTrace{
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				Tracer: &tracer,
				StateOverrides: &etdapi.StateOverride{
					randomAccounts[2].addr: etdapi.OverrideAccount{
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(result, &etdapi.ExecutionResult{
		Gas:         params.TxGas,
		Failed:      false,
		ReturnValue: "",
		StructLogs:  []etdapi.StructLogRes{},
