...
func TestFastSyncDisabling65(t *testing.T) { testFastSyncDisabling(t, etd.ETD65) }
func TestFastSyncDisabling66(t *testing.T) { testFastSyncDisabling(t, etd.ETD66) }
...
func testFastSyncDisabling(t *testing.T, protocol uint) {
	...
	emptyPeer := etd.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), emptyPipe, empty.txpool)
	fullPeer := etd.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), fullPipe, full.txpool)
	defer emptyPeer.Close()
	defer fullPeer.Close()

	go empty.handler.runEthPeer(emptyPeer, func(peer *etd.Peer) error {
		return etd.Handle((*etdHandler)(empty.handler), peer)
	})
	go full.handler.runEthPeer(fullPeer, func(peer *etd.Peer) error {
		return etd.Handle((*etdHandler)(full.handler), peer)
	})
