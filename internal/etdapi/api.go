...
import (
	...
	"github.com/crypyto-panel/go-etherdata/accounts/scwallet"
...
// PublicEtherdataAPI provides an API to access Etherdata related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEtherdataAPI struct {
	b Backend
}

// NewPublicEtherdataAPI creates a new Etherdata protocol API.
func NewPublicEtherdataAPI(b Backend) *PublicEtherdataAPI {
	return &PublicEtherdataAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicEtherdataAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicEtherdataAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicEtherdataAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Etherdata ECDSA signature for:
// keccack256("\x19Etherdata Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/crypyto-panel/go-etherdata/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with etd_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Etherdata Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/crypyto-panel/go-etherdata/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Etherdata signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and getd.
...
// PublicBlockChainAPI provides an API to access the Etherdata blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Etherdata blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current etherdata chain config.
...
// See: https://github.com/crypyto-panel/go-etherdata/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/crypyto-panel/go-etherdata/wiki/wiki/JSON-RPC#etd_sign
...
// PublicDebugAPI is the collection of Etherdata APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", etdash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Etherdata APIs exposed over the private
// debugging endpoint.
...
// Version returns the current etherdata protocol version.
