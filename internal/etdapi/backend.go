...
// Package etdapi implements the general Etherdata API functions.
package etdapi
...
type Backend interface {
	// General Etherdata API
	Downloader() *downloader.Downloader
	SuggestGasTipCap(ctx context.Context) (*big.Int, error)
	ChainDb() etddb.Database
	AccountManager() *accounts.Manager
	ExtRPCEnabled() bool
	RPCGasCap() uint64        // global gas cap for etd_call over rpc: DoS protection
	...
	return []rpc.API{
		{
			Namespace: "etd",
			Version:   "1.0",
			Service:   NewPublicEtherdataAPI(apiBackend),
			Public:    true,
		}, {
			Namespace: "etd",
			Version:   "1.0",
