...
func NewSyncBloom(memory uint64, database etddb.Iteratee) *SyncBloom {
...
func (b *SyncBloom) init(database etddb.Iteratee) {
...
func (b *SyncBloom) Contains(hash []byte) bool {
	...
	if atomic.LoadUint32(&b.inited) == 0 {
		// We didn't load all the trie nodes from the previous run of Getd yet. As
