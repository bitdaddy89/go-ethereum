...
var errEthashStopped = errors.New("etdash stopped")

// API exposes etdash related methods for the RPC interface.
type API struct {
	etdash *Ethash
}
...
func (api *API) GetWork() ([4]string, error) {
	...
	select {
	case api.etdash.remote.fetchWorkCh <- &sealWork{errc: errc, res: workCh}:
	case <-api.etdash.remote.exitCh:
...
func (api *API) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	...
	select {
	case api.etdash.remote.submitWorkCh <- &mineResult{
		nonce:     nonce,
		mixDigest: digest,
		hash:      hash,
		errc:      errc,
...
func (api *API) SubmitHashrate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.etdash.remote.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.etdash.remote.exitCh:
		return false
	}
...
func (api *API) GetHashrate() uint64 {
	return uint64(api.etdash.Hashrate())
}
