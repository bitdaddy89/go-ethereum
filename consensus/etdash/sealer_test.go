...
import (
	...
	"github.com/crypyto-panel/go-etherdata/internal/testlog"
...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom etdash engine.
	etdash := NewTester([]string{server.URL}, false)
	defer etdash.Close()
	...
	etdash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := etdash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	etdash := New(config, []string{server.URL}, false)
	defer etdash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom etdash engine.
	etdash := NewTester([]string{server.URL}, false)
	etdash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer etdash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			etdash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, etdash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch etdash result timeout", id+1)
		}
	}
}
