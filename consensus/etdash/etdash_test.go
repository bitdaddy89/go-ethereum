...
// Tests that etdash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	etdash := NewTester(nil, false)
	defer etdash.Close()

	results := make(chan *types.Block)
	err := etdash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := etdash.verifySeal(nil, header, false); err != nil {
...
// It reproduces https://github.com/crypyto-panel/go-etherdata/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "etdash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := etdash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	etdash.Seal(nil, block, results, nil)
...
func TestHashrate(t *testing.T) {
	...
	if tot := etdash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	if _, err := api.GetWork(); err != errEthashStopped {
		t.Error("expect to return an error to indicate etdash is stopped")
	}

	if res := api.SubmitHashrate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped etdash")
	}
}
