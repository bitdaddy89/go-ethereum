...
Clef can be used to sign transactions and data and is meant as a(n eventual) replacement for Getd's account management. This allows DApps to not depend on Getd's account management. When a DApp wants to sign data (or a transaction), it can send the content to Clef, which will then provide the user with context and asks for permission to sign the content. If the users grants the signing request, Clef will send the signature back to the DApp.

This setup allows a DApp to connect to a remote Etherdata node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Etherdata node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.etherdata/keystore")
...
The general flow for signing a transaction using e.g. Getd is as follows:
![image](sign_flow.png)

In this case, `getd` would be started with `--signer http://localhost:8550` and would relay requests to `etd.sendTransaction`.
...
* Getd todos
    ...
    - Relay: Getd should be started in `getd --signer localhost:8550`.
    - Currently, the Getd APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The Getd API should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
    * [x] An encrypted key-value storage should be implemented.
    * See [rules.md](rules.md) for more info about this.
* Another potential thing to introduce is pairing.
  * To prevent spurious requests which users just accept, implement a way to "pair" the caller with the signer (external API).
  * Thus Getd/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
Clef listens to HTTP requests on `http.addr`:`http.port` (or to IPC on `ipcpath`), with the same JSON-RPC standard as Getd. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
...
The signer will generate a new private key, encrypt it according to [web3 keystore spec](https://github.com/crypyto-panel/go-etherdata/wiki/wiki/Web3-Secret-Storage-Definition) and store it in the keystore directory.  
...
#### Arguments
  - content type [string]: type of signed data
     ...
     - `application/clique`: [clique](https://github.com/crypyto-panel/go-etherdata/EIPs/issues/225) headers
...
#### Sign data
   Signs a chunk of structured data conformant to [EIP-712](https://github.com/crypyto-panel/go-etherdata/EIPs/blob/master/EIPS/eip-712.md) and returns the calculated signature.
  ...
  "params": [
    {
      "accounts": [
        {
          "url": "keystore:///home/bazonk/.etherdata/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "url": "keystore:///home/bazonk/.etherdata/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
...
* A Graphical UI MUST show the blocky-identicon for etherdata addresses.
...
| Clef UI| https://github.com/crypyto-panel/go-etherdata/clef-ui| Golang/QT-based| :+1:| :+1:| :x:| :+1:| :+1:| :x: |  :+1: (approve tx only)|
