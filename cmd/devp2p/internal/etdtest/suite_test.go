...
func TestEthSuite(t *testing.T) {
	getd, err := runGetd()
	if err != nil {
		t.Fatalf("could not run getd: %v", err)
	}
	defer getd.Close()

	suite, err := NewSuite(getd.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGetd creates and starts a getd node
func runGetd() (*node.Node, error) {
...
func setupGetd(stack *node.Node) error {
	chain, err := loadChain(halfchainFile, genesisFile)
	if err != nil {
		return err
	}

	backend, err := etd.New(stack, &etdconfig.Config{
