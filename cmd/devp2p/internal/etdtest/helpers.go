...
// Is_66 checks if the node supports the etd66 protocol version,
...
// returning the created Conn with additional etd66 capabilities if
// successful
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "etd", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("unexpected etd protocol version")
		}
		return nil
	default:
...
// negotiateEthProtocol sets the Conn's etd protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "etd" {
			continue
		}
...
loop:
	...
	// make sure etd protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("etd protocol version must be set in Conn")
	}
	if status == nil {
...
// readAndServe66 serves etd66 GetBlockHeaders requests while waiting
// on another message from the node.
func (c *Conn) readAndServe66(chain *Chain, timeout time.Duration) (uint64, Message) {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			...
			resp := &etd.BlockHeadersPacket66{
				RequestId:          reqID,
				BlockHeadersPacket: etd.BlockHeadersPacket(headers),
			}
...
func (c *Conn) headersRequest(request *GetBlockHeaders, chain *Chain, isEth66 bool, reqID uint64) (BlockHeaders, error) {
	...
	// if on etd66 connection, perform etd66 GetBlockHeaders request
	if isEth66 {
...
// getBlockHeaders66 executes the given `GetBlockHeaders` request over the etd66 protocol.
func getBlockHeaders66(chain *Chain, conn *Conn, request *GetBlockHeaders, id uint64) (BlockHeaders, error) {
	// write request
	packet := etd.GetBlockHeadersPacket(*request)
	req := &etd.GetBlockHeadersPacket66{
		RequestId:             id,
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block, isEth66 bool) error {
	...
	for {
		...
		if isEth66 {
			requestID := uint64(54)
			headers, err = conn.headersRequest(req, s.chain, etd66, requestID)
		} else {
...
func (s *Suite) hashAnnounce(isEth66 bool) error {
	...
	if isEth66 {
		...
		case GetBlockHeaders:
			...
			resp := &etd.BlockHeadersPacket66{
				RequestId: id,
				BlockHeadersPacket: etd.BlockHeadersPacket{
					nextBlock.Header(),
				},
			}
