...
// NewSuite creates and returns a new etd-test suite that can
...
func (s *Suite) Eth66Tests() []utesting.Test {
	return []utesting.Test{
		// only proceed with etd66 test suite if node supports etd 66 protocol
		{Name: "TestStatus66", Fn: s.TestStatus66},
		{Name: "TestGetBlockHeaders66", Fn: s.TestGetBlockHeaders66},
		{Name: "TestSimultaneousRequests66", Fn: s.TestSimultaneousRequests66},
		{Name: "TestSameRequestID66", Fn: s.TestSameRequestID66},
...
var (
	etd66 = true  // indicates whether suite should negotiate etd66 connection
	etd65 = false // indicates whether suite should negotiate etd65 connection or below.
)
...
// a status message with it on the etd66 protocol.
...
func (s *Suite) TestGetBlockHeaders(t *utesting.T) {
	...
	headers, err := conn.headersRequest(req, s.chain, etd65, 0)
	if err != nil {
...
// an etd66 `GetBlockHeaders` request and that the response is accurate.
func (s *Suite) TestGetBlockHeaders66(t *utesting.T) {
	...
	headers, err := conn.headersRequest(req, s.chain, etd66, 33)
	if err != nil {
...
func (s *Suite) TestSimultaneousRequests66(t *utesting.T) {
	...
	// create two requests
	req1 := &etd.GetBlockHeadersPacket66{
		RequestId: uint64(111),
		GetBlockHeadersPacket: &etd.GetBlockHeadersPacket{
			Origin: etd.HashOrNumber{
				Hash: s.chain.blocks[1].Hash(),
			},
			Amount:  2,
	...
	req2 := &etd.GetBlockHeadersPacket66{
...
func (s *Suite) TestSameRequestID66(t *utesting.T) {
	...
	// create requests
	reqID := uint64(1234)
	request1 := &etd.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &etd.GetBlockHeadersPacket{
			Origin: etd.HashOrNumber{
				Number: 1,
	...
	request2 := &etd.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &etd.GetBlockHeadersPacket{
			Origin: etd.HashOrNumber{
				Number: 33,
...
func (s *Suite) TestZeroRequestID66(t *utesting.T) {
	...
	headers, err := conn.headersRequest(req, s.chain, etd66, 0)
	if err != nil {
...
func (s *Suite) TestGetBlockBodies66(t *utesting.T) {
	...
	// create block bodies request
	req := &etd.GetBlockBodiesPacket66{
		RequestId: uint64(55),
		GetBlockBodiesPacket: etd.GetBlockBodiesPacket{
			s.chain.blocks[54].Hash(),
			s.chain.blocks[75].Hash(),
		},
	}
...
func (s *Suite) TestBlockHashAnnounce(t *utesting.T) {
	if err := s.hashAnnounce(etd65); err != nil {
...
func (s *Suite) TestBlockHashAnnounce66(t *utesting.T) {
	if err := s.hashAnnounce(etd66); err != nil {
		t.Fatalf("block hash announcement failed: %v", err)
	}
}
...
func (s *Suite) TestMaliciousHandshake(t *utesting.T) {
	if err := s.maliciousHandshakes(t, etd65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousHandshake66(t *utesting.T) {
	if err := s.maliciousHandshakes(t, etd66); err != nil {
		t.Fatal(err)
	}
}
...
// difficulty over the etd66 protocol.
...
func (s *Suite) TestTransaction(t *utesting.T) {
	if err := s.sendSuccessfulTxs(t, etd65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestTransaction66(t *utesting.T) {
	if err := s.sendSuccessfulTxs(t, etd66); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousTx(t *utesting.T) {
	if err := s.sendMaliciousTxs(t, etd65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousTx66(t *utesting.T) {
	if err := s.sendMaliciousTxs(t, etd66); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestLargeTxRequest66(t *utesting.T) {
	...
	getTxReq := &etd.GetPooledTransactionsPacket66{
		RequestId:                   1234,
