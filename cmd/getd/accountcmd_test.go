...
func TestAccountListEmpty(t *testing.T) {
	getd := runGetd(t, "account", "list")
	getd.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	getd := runGetd(t, "account", "list", "--datadir", datadir)
	defer getd.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	getd := runGetd(t, "account", "new", "--lightkdf")
	defer getd.ExpectExit()
	getd.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	getd := runGetd(t, "account", "import", keyfile, "-password", passwordFile)
	defer getd.ExpectExit()
	getd.Expect(expected)
}
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "sometding"}}
Repeat password: {{.InputLine "sometding else"}}
...
func TestWalletImport(t *testing.T) {
	getd := runGetd(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer getd.ExpectExit()
	getd.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(getd.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	getd := runMinimalGetd(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(getd.StderrText(), m) {
...
// https://github.com/crypyto-panel/go-etherdata/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	getd.SetTemplateFunc("keypath", func(file string) string {
