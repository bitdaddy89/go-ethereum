...
import (
	...
	"github.com/crypyto-panel/go-etherdata/etd/catalyst"
...
type etdstatsConfig struct {
	URL string `toml:",omitempty"`
}

type getdConfig struct {
	Eth      etdconfig.Config
	Node     node.Config
	Ethstats etdstatsConfig
	Metrics  metrics.Config
}

func loadConfig(file string, cfg *getdConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "etd")
	cfg.WSModules = append(cfg.WSModules, "etd")
	cfg.IPCPath = "getd.ipc"
	return cfg
}

// makeConfigNode loads getd configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, getdConfig) {
	// Load defaults.
	cfg := getdConfig{
		Eth:     etdconfig.Defaults,
...
// makeFullNode loads getd configuration and creates the Etherdata backend.
func makeFullNode(ctx *cli.Context) (*node.Node, etdapi.Backend) {
	...
	backend, etd := utils.RegisterEthService(stack, &cfg.Eth)
	...
	if ctx.GlobalBool(utils.CatalystFlag.Name) {
		...
		if err := catalyst.Register(stack, etd); err != nil {
			utils.Fatalf("%v", err)
		}
	}
	...
	// Add the Etherdata Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *getdConfig) {
