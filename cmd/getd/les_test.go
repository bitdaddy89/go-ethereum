...
func (g *getdrpc) killAndWait() {
	g.getd.Kill()
	g.getd.WaitExit()
}

func (g *getdrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.getd.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *getdrpc) addPeer(peer *getdrpc) {
	g.getd.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.getd.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.getd.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.getd.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.getd.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *getdrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *getdrpc) waitSynced() {
	// Check if it's synced now
	var result interface{}
	g.callRPC(&result, "etd_syncing")
	syncing, ok := result.(bool)
	if ok && !syncing {
		g.getd.Logf("%v already synced", g.name)
		return
	}

	// Actually wait, subscribe to the event
	ch := make(chan interface{})
	sub, err := g.rpc.Subscribe(context.Background(), "etd", ch, "syncing")
	if err != nil {
		g.getd.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.getd.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.getd.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.getd.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.getd.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGetdWithIpc(t *testing.T, name string, args ...string) *getdrpc {
	ipcName := fmt.Sprintf("getd-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	ipcpath := ipcEndpoint(ipcName, g.getd.Datadir)
	// We can't know exactly how long getd will take to start, so we try 10
...
func initGetd(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing getd: %v ", args)
	g := runGetd(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *getdrpc {
	datadir := initGetd(t)
	t.Logf("Importing keys to getd")
	runGetd(t, "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGetdWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *getdrpc {
	datadir := initGetd(t)
	return startGetdWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
