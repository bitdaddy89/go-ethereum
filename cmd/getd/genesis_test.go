...
// Tests that initializing Getd with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		...
		runGetd(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		getd := runGetd(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		getd.ExpectRegexp(tt.result)
		getd.ExpectExit()
	}
}
