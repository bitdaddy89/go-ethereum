...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "getd-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "getd-test" in runGetd.
	reexec.Register("getd-test", func() {
...
// spawns getd with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGetd(t *testing.T, args ...string) *testgetd {
	tt := &testgetd{}
	...
	if tt.Datadir == "" {
		...
		// Remove the temporary datadir if sometding fails below.
		defer func() {
			if t.Failed() {
				tt.Cleanup()
			}
	...
	// Boot "getd". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("getd-test", args...)
