...
*For a more detailed introduction to checkpoint, please see the LES [spec](https://github.com/crypyto-panel/go-etherdata/devp2p/blob/master/caps/les.md).*
...
There are several hardcoded checkpoints in the [source code](https://github.com/crypyto-panel/go-etherdata/blob/master/params/config.go#L38) of the go-etherdata project. These checkpoints are updated by go-etherdata developers when new versions of software are released. Because light client users trust Getd developers to some extent, hardcoded checkpoints in the code can also be considered correct.

#### Checkpoint oracle

Hardcoded checkpoints can solve the problem of verifying the correctness of checkpoints (although this is a more centralized solution). But the pain point of this solution is that developers can only update checkpoints when a new version of software is released. In addition, light client users usually do not keep the Getd version they use always up to date. So hardcoded checkpoints used by users are generally stale. Therefore, it still needs to download a large amount of blockchain data during synchronization.
...
```shell
go get github.com/crypyto-panel/go-etherdata/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://getd.etherdata.org/docs/clef/tutorial) .
...
*It is worth noting that the connected Getd node can be a fullnode or a light client. If it is fullnode, you must enable the LES protocol. E.G. add `--light.serv 50` to the startup command line flags*.
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/crypyto-panel/go-etherdata/blob/master/params/config.go#L251).*
...
* Start getd with the modified configuration file
