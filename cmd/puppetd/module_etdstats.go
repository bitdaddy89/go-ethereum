...
// etdstatsDockerfile is the Dockerfile required to build an etdstats backend
// and associated monitoring site.
var etdstatsDockerfile = `
FROM puppetd/etdstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// etdstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an etdstats monitoring site.
var etdstatsComposefile = `
version: '2'
services:
  etdstats:
    build: .
    image: {{.Network}}/etdstats
    container_name: {{.Network}}_etdstats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new etdstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(etdstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(etdstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the etdstats service
	if nocache {
...
// etdstatsInfos is returned from an etdstats status check to allow reporting
// various configuration parameters.
type etdstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *etdstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an etdstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*etdstatsInfos, error) {
	// Inspect a possible etdstats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_etdstats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
