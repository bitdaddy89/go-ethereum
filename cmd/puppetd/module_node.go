...
// nodeDockerfile is the Dockerfile required to run an Etherdata node.
var nodeDockerfile = `
FROM etherdata/client-go:latest
...
{{end}}
RUN \
  echo 'getd --cache 512 init /genesis.json' > getd.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.etherdata/keystore/ && cp /signer.json /root/.etherdata/keystore/' >> getd.sh && \{{end}}
	echo $'exec getd --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --etdstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> getd.sh

ENTRYPOINT ["/bin/sh", "getd.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Etherdata node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.etherdata{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.etdash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
// deployNode deploys a new Etherdata node container to a remote machine via SSH,
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethashdir":  config.etdashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Ethstats":   config.etdstats[:strings.Index(config.etdstats, ":")],
		"Etherbase":  config.etherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.etdstats,
	}
	if info.gasTarget > 0 {
		...
		if info.etherbase != "" {
			...
			report["Ethash directory"] = info.etdashdir
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 getd --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.etherdata"],
		etdashdir:  infos.volumes["/root/.etdash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		etdstats:   infos.envvars["STATS_NAME"],
