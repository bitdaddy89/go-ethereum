...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of etdash, we're pretty much done
...
// importGenesis imports a Getd genesis spec into puppetd.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-aletd.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppetd and Getd
		getdJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile(getdJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", getdJson)

		// Export the genesis spec used by Aletd (formerly C++ Etherdata)
		if spec, err := newAletdGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Aletd chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "aletd", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly EtherdataJ)
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not sometding I can do")
		return
	}
}
