...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"etdstats", "explorer", "faucet"} {
	...
	// If we have etdstats running, ask whether to make the secret public or not
	if w.conf.etdstats != "" {
		fmt.Println()
		fmt.Println("Include etdstats secret on dashboard (y/n)? (default = yes)")
