...
// makeWizard creates and returns a new puppetd wizard.
...
// setting up a new or managing an existing Etherdata private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppetd, your Etherdata private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Etherdata network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and etdstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppetd uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Etherdata network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppetd", w.network)
