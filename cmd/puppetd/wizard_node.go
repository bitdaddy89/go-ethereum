...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.etdstats == "" {
		log.Error("No etdstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.etdashdir == "" {
			fmt.Printf("Where should the etdash mining DAGs be stored on the remote machine?\n")
			infos.etdashdir = w.readString()
		} else {
			fmt.Printf("Where should the etdash mining DAGs be stored on the remote machine? (default = %s)\n", infos.etdashdir)
			infos.etdashdir = w.readDefaultString(infos.etdashdir)
		}
	}
	...
	if infos.etdstats == "" {
		...
		infos.etdstats = w.readString() + ":" + w.conf.etdstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.etdstats)
		infos.etdstats = w.readDefaultString(infos.etdstats) + ":" + w.conf.etdstats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Etherdata node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
