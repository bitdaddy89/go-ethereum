...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for etdstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["etdstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["etdstats"] = infos.Report()
		etdstats = infos.config
	}
