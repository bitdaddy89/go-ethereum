...
import (
	...
	math2 "github.com/crypyto-panel/go-etherdata/common/math"
...
// aletdGenesisSpec represents the genesis specification format used by the
// C++ Etherdata implementation.
type aletdGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*aletdGenesisSpecAccount `json:"accounts"`
}

// aletdGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type aletdGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance,omitempty"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *aletdGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// aletdGenesisSpecBuiltin is the precompiled contract definition.
type aletdGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock *hexutil.Big                   `json:"startingBlock,omitempty"`
	Linear        *aletdGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type aletdGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAletdGenesisSpec converts a go-etherdata genesis block into a Aletd-specific
// chain specification format.
func newAletdGenesisSpec(network string, genesis *core.Genesis) (*aletdGenesisSpec, error) {
	// Only etdash is currently supported between go-etherdata and aletd
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Aletd format
	spec := &aletdGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	// config but aletd will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.Params.BlockReward = (*hexutil.Big)(etdash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &aletdGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &aletdGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &aletdGenesisSpecBuiltin{Name: "sha256",
		Linear: &aletdGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &aletdGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &aletdGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &aletdGenesisSpecBuiltin{Name: "identity",
		Linear: &aletdGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &aletdGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &aletdGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &aletdGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &aletdGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &aletdGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &aletdGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &aletdGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Aletd hardcoded the gas policy
		spec.setPrecompile(7, &aletdGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Aletd hardcoded the gas policy
		spec.setPrecompile(9, &aletdGenesisSpecBuiltin{
...
func (spec *aletdGenesisSpec) setPrecompile(address byte, data *aletdGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*aletdGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &aletdGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *aletdGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*aletdGenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-etherdata/pull/11039,
...
// newParityChainSpec converts a go-etherdata genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only etdash is currently supported between go-etherdata and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Engine.Ethash.Params.BlockReward["0x0"] = hexutil.EncodeBig(etdash.FrontierBlockReward)

	// Homestead
	spec.Engine.Ethash.Params.HomesteadTransition = hexutil.Uint64(genesis.Config.HomesteadBlock.Uint64())

	// Tangerine Whistle : 150
	// https://github.com/crypyto-panel/go-etherdata/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/crypyto-panel/go-etherdata/EIPs/blob/master/EIPS/eip-607.md
	...
	// getd has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Etherdata.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.Etherdata.MixHash = genesis.Mixhash[:]
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(etdash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(etdash.ConstantinopleBlockReward)
...
// pyEtherdataGenesisSpec represents the genesis specification format used by the
// Python Etherdata implementation.
type pyEtherdataGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyEtherdataGenesisSpec converts a go-etherdata genesis block into a Parity specific
// chain specification format.
func newPyEtherdataGenesisSpec(network string, genesis *core.Genesis) (*pyEtherdataGenesisSpec, error) {
	// Only etdash is currently supported between go-etherdata and pyetherdata
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyEtherdataGenesisSpec{
