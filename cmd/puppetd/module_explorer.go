...
FROM puppetd/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'getd --cache 512 init /genesis.json' > explorer.sh && \
  echo $'getd --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --etdstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,etd,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec getd --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --etdstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,etd,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"EthPort":     config.node.port,
		"EthName":     config.node.etdstats[:strings.Index(config.node.etdstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Etherdata listener port ": strconv.Itoa(info.node.port),
		"Ethstats username":       info.node.etdstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["ETD_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.etherdata"],
			port:     infos.portmap[infos.envvars["ETD_PORT"]+"/tcp"],
			etdstats: infos.envvars["ETD_NAME"],
		},
