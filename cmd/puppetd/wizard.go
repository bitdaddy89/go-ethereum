...
// config contains all the configurations needed by puppetd that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	etdstats  string   // Ethstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppetd configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Etherdata services known to be running on servers
...
// converts it to an Etherdata address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by etdstats, which compares IPs textually, not by value.
