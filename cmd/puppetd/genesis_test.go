...
// Tests the go-etherdata to Aletd chainspec conversion for the Stureby testnet.
func TestAletdSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_getd.json")
	if err != nil {
	...
	spec, err := newAletdGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_aletd.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &aletdGenesisSpec{}
...
// Tests the go-etherdata to Parity chainspec conversion for the Stureby testnet.
