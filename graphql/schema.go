...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Etherdata address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Etherdata account at a particular block.
    type Account {
    ...
    # Transaction is an Etherdata transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Etherdata account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Etherdata account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Etherdata block by number or by hash. If neither is
