...
# 1. install getd for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for getd
# 5. create getd, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable ETHERDATA_SOCKET
...
!define GROUPNAME "Etherdata"
!define APPNAME "Getd"
!define DESCRIPTION "Official Go implementation of the Etherdata protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since getd is installed global
