...
func ReadSnapshotDisabled(db etddb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db etddb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db etddb.KeyValueWriter) {
...
func ReadSnapshotRoot(db etddb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db etddb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db etddb.KeyValueWriter) {
...
func ReadAccountSnapshot(db etddb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db etddb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db etddb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db etddb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db etddb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db etddb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db etddb.Iteratee, accountHash common.Hash) etddb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db etddb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db etddb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db etddb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db etddb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db etddb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db etddb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db etddb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db etddb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db etddb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db etddb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db etddb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSnapshotSyncStatus(db etddb.KeyValueWriter) {
