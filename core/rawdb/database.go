...
import (
	...
	"github.com/crypyto-panel/go-etherdata/etddb/leveldb"
...
func NewDatabase(db etddb.KeyValueStore) etddb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db etddb.KeyValueStore, freezer string, namespace string, readonly bool) (etddb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains sometding, ensure that the genesis blocks
...
func NewMemoryDatabase() etddb.Database {
...
func NewMemoryDatabaseWithCap(size int) etddb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string, readonly bool) (etddb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string, readonly bool) (etddb.Database, error) {
...
func InspectDatabase(db etddb.Database, keyPrefix, keyStart []byte) error {
