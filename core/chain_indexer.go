...
type ChainIndexer struct {
	chainDb  etddb.Database      // Chain database to index the data from
	indexDb  etddb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb etddb.Database, indexDb etddb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
