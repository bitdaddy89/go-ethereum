...
// Package core implements the Etherdata consensus protocol.
package core
...
type BlockChain struct {
	...
	db     etddb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Etherdata Validator and
// Processor.
func NewBlockChain(db etddb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (etdash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) SetHeadBeyondRoot(head uint64, root common.Hash) (uint64, error) {
	...
	updateFn := func(db etddb.KeyValueWriter, header *types.Header) (uint64, bool) {
		...
		// between the stateful-block and the setdead target.
		var wipe bool
		if head+1 < frozen {
	...
	delFn := func(db etddb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. Getd is initialized with
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// tx indexes)
			if batch.ValueSize() >= etddb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - etddb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have sometding meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
...
// sync, Getd will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Getd with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Getd to a new version which supports transaction
