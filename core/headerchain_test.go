...
func TestHeaderInsertion(t *testing.T) {
	...
	hc, err := NewHeaderChain(db, params.AllEthashProtocolChanges, etdash.NewFaker(), func() bool { return false })
	if err != nil {
		t.Fatal(err)
	}
	// chain A: G->A1->A2...A128
	chainA := makeHeaderChain(genesis.Header(), 128, etdash.NewFaker(), db, 10)
	// chain B: G->A1->B2...B128
	chainB := makeHeaderChain(chainA[0], 128, etdash.NewFaker(), db, 10)
