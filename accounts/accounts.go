...
// Package accounts implements high level Etherdata account management.
package accounts
...
// Account represents an Etherdata account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Etherdata account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain etherdata.ChainStateReader)
	...
	// by the Etherdata prefix scheme
	// It looks up the account specified either solely via its address contained within,
	// or optionally with the aid of any location metadata from the embedded URL field.
	//
	// If the wallet requires additional authentication to sign the request (e.g.
...
//   keccak256("\x19Etherdata Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Etherdata Signed Message:\n%d%s", len(data), string(data))
