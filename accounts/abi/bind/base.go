...
// valid Etherdata transaction.
type TransactOpts struct {
	From   common.Address // Etherdata account to send the transaction from
...
// Etherdata network. It contains a collection of methods that are used by the
// higher level contract bindings to operate.
type BoundContract struct {
	address    common.Address     // Deployment address of the contract on the Etherdata blockchain
	abi        abi.ABI            // Reflect based ABI to access the correct Etherdata methods
...
// DeployContract deploys a contract onto the Etherdata blockchain and binds the
...
func (c *BoundContract) Call(opts *CallOpts, results *[]interface{}, method string, params ...interface{}) error {
	...
	var (
		msg    = etherdata.CallMsg{From: opts.From, To: &c.address, Data: input}
...
func (c *BoundContract) transact(opts *TransactOpts, contract *common.Address, input []byte) (*types.Transaction, error) {
	...
	if gasLimit == 0 {
		...
		msg := etherdata.CallMsg{From: opts.From, To: contract, GasPrice: opts.GasPrice, GasTipCap: opts.GasTipCap, GasFeeCap: opts.GasFeeCap, Value: value, Data: input}
