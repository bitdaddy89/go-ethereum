...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != etherdata.NotFound {
		t.Fatalf("err should be `etherdata.NotFound` but received %v", err)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	var cases = []struct {
		name        string
		message     etherdata.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", etherdata.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", etherdata.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", etherdata.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", etherdata.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", etherdata.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", etherdata.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
...
func TestPendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, etherdata.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, etherdata.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
contract Reverter {
    ...
    function noRevert() public pure {
        assembly {
            // Assembles sometding that looks like require(false, "some error") but is not reverted
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, etherdata.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, etherdata.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
