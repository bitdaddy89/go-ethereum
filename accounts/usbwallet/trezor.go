...
import (
	...
	"github.com/crypyto-panel/go-etherdata/accounts/usbwallet/trezor"
...
// or whether the Etherdata app was not started on it.
...
// Etherdata address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EtherdataAddress)
	if _, err := w.trezorExchange(&trezor.EtherdataGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.EtherdataSignTx{
	...
	response := new(trezor.EtherdataTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EtherdataTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
