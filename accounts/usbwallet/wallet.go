...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Etherdata
...
type wallet struct {
	...
	deriveChain     etherdata.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Etherdata account
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain etherdata.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
