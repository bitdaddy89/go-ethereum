...
func newPruner(db etddb.Database, indexers ...*core.ChainIndexer) *pruner {
	pruner := &pruner{
		db:       db,
		indexers: indexers,
...
// historical chain data. Notably, whenever Getd restarts, it will iterate
...
func (p *pruner) loop() {
	...
	// Note, if some indexers don't support pruning(e.g. etd.BloomIndexer),
