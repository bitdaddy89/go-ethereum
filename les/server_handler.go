...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb etddb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Etherdata peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light Etherdata handshake failed", "err", err)
		return err
	}
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light etherdata response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Etherdata message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Etherdata message arrived", "code", msg.Code, "bytes", msg.Size)
