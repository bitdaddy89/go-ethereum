...
type LesServer struct {
	...
	archiveMode bool // Flag whether the etherdata node runs in archive mode.
...
func NewLesServer(node *node.Node, e etdBackend, config *etdconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "etd/db/lesserver/", false)
	if err != nil {
		return nil, err
	}
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "Etherdata light client service")
