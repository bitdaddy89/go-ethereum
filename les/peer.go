...
import (
	...
	"github.com/crypyto-panel/go-etherdata/les/utils"
...
// PeerInfo represents a short summary of the `etd` sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Etherdata protocol version negotiated
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// as well. Check http://eips.etherdata.org/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local etherdata node is running in archive mode, advertise ourselves we have
...
// participating in the Light Etherdata sub-protocol.
...
// for us(server) but for other protocols(e.g. ETD) upon the devp2p they
// may be useful.
